...
void printProduct(void* element)
{
	Product* product = *(Product**)element;
	char* dateStr = getDateStr(&product->expiryDate);
	printf("%-20s %-10s\t", product->name, product->barcode);
	printf("%-20s %5.2f %13d %7s %15s\n", typeStr[product->type], product->price, product->count, " ", dateStr);
	free(dateStr);
}
...
int saveProductToFile(Product* product, FILE* pFile)
{
	if (!pFile)
	{
		return 0;
	}
	if (fwrite(product, sizeof(Product), 1, pFile) != 1)
	{
		return 0;
	}
	return 1;
